...
export default {
    ...
    methods:{
        ...
        // 新密码失去焦点
        newpdB:function(){
            if(this.inputnewpd){//自己不为空
                if(this.inputoldpd){//旧密码不为空
                    if(/^(?=.*\d)(?=.*[A-Z])(?=.*[a-z]).{8,20}$/.test(this.inputnewpd)&&this.inputoldpd!=this.inputnewpd){//符合正则且与旧密码不同
                        this.colorNew='#5d95e8';
                        return;
                    }
                    if(!/^(?=.*\d)(?=.*[A-Z])(?=.*[a-z]).{8,20}$/.test(this.inputnewpd)){//不符合正则
                        this.errorshow=true;
                    ...
                    if(this.inputoldpd==this.inputnewpd){//与旧密码不同
                        this.errorshow=true;
        ...
        // 再次输入密码失去焦点
        againB:function(){
            if(this.inputagain){//不为空
                if(this.inputnewpd){//新密码不为空
                    if(/^(?=.*\d)(?=.*[A-Z])(?=.*[a-z]).{8,20}$/.test(this.inputagain)&&this.inputagain==this.inputnewpd){//密码符合正则且两次新密码输入相同
                        this.colorAgain='#5d95e8';
                        return;
                    }
                    if(!/^(?=.*\d)(?=.*[A-Z])(?=.*[a-z]).{8,20}$/.test(this.inputagain)){//密码不符合正则
                        this.errorshow=true;
                        this.error='请输入（8-20位）数字、大小写字母的新密码';
                    ...
                    if(this.inputagain!=this.inputnewpd){//两次新密码输入不同
                        this.errorshow=true;
        ...
        store:function(){
            if(this.getName){
                if(this.colorAgain=='#5d95e8'&&this.colorNew=='#5d95e8'&&this.colorOld=='#5d95e8'){
                    var that=this;
                    that.ajax.post('/xinda-api/sso/change-pwd',
                    that.qs.stringify({
                        oldPwd:md5(that.inputoldpd),
                        newPwd:md5(that.inputnewpd)
                    })).then(function(data){
                        if(data.data.status==1){//修改成功
                            that.errorshow=true;
